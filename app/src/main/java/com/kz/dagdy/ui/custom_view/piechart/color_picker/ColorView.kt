...
class ColorView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val size =
            ColorPickerUtils.setWrapSize(100.toPx, 100.toPx, widthMeasureSpec, heightMeasureSpec)
        setMeasuredDimension(size.width, size.height)

        roundedClipPath.reset()
        roundedClipPath.addRoundRect(
            0F,
            0F,
            size.width.toFloat(),
            size.height.toFloat(),
    ...
    private val roundSize = resources.getDimensionPixelSize(R.dimen.dp_22).toFloat()
