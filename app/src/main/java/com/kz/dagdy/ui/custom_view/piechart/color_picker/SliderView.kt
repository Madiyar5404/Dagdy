...
open class SliderView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    private val wheelPaint = ColorPickerUtils.fillPaint(Color.WHITE).apply {
        isAntiAlias = true
        setShadowLayer(
            5f,
            0f, 0f,
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val size = ColorPickerUtils.setWrapSize(
            100.toPx + paddingLeft + paddingRight,
            20.toPx + paddingTop + paddingBottom,
            widthMeasureSpec,
            heightMeasureSpec
        )
    ...
    private fun drawWheel(canvas: Canvas) {
        ...
        canvas.drawCircle(
            getRadius() + paddingLeft + maxTrackSize() * currentRatio,
            halfHeight,
            getRadius().toFloat(),
            wheelPaint
        )
